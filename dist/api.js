...
export class VoiceAIClient {
    ...
    async listVoices(options) {
        if (this.mock)
            return this.mockListVoices(options);
        // Check cache
        ...
        const rawVoices = Array.isArray(json)
            ? json
            : (json.voices ?? []);
    ...
    async generateSpeech(request, tmpDir) {
        if (this.mock)
            return this.mockGenerateSpeech(request);
    ...
    async callTtsEndpoint(text, opts) {
        ...
        if (!res.ok) {
            ...
            if (res.status === 401)
                throw new Error('Voice.ai: Invalid or missing API key (401).');
            if (res.status === 402)
                throw new Error('Voice.ai: Insufficient credits (402). Check your dashboard.');
            if (res.status === 429)
                throw new Error('Voice.ai: Rate limited (429). Wait and retry.');
    ...
    mockListVoices(options) {
        return this.filterVoices({ voices: POPULAR_VOICES, total: POPULAR_VOICES.length }, options);
    }
