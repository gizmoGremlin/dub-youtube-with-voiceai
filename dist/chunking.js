...
export function autoChunk(content, maxChars) {
...
export async function chunkScript(content, options) {
    // 1. Parse into raw sections
    let rawSections;
    if (options.mode === 'headings') {
        const headingSections = parseMarkdownHeadings(content);
        const hasRealHeadings = headingSections.some((s) => s.title !== 'Preamble' && s.title !== 'Untitled Section');
