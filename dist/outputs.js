...
import { writeOutputFile, youtubeTimestamp, srtTimestamp, formatDuration, } from './utils.js';
export function buildManifest(title, voiceId, voiceName, template, language, mock, results) {
...
export function buildChapters(timeline) {
    if (!timeline.has_durations)
        return null;
    const lines = timeline.segments.map((s) => `${youtubeTimestamp(s.start_seconds)} ${s.title}`);
    return lines.join('\n');
}
/* ------------------------------------------------------------------ */
/*  Captions (SRT)                                                     */
/* ------------------------------------------------------------------ */
export function buildCaptionsSrt(timeline, segments) {
    if (!timeline.has_durations)
        return null;
...
/* ------------------------------------------------------------------ */
export function buildDescription(title, chapters) {
    let desc = `${title}\n${'='.repeat(title.length)}\n\n`;
    if (chapters) {
        desc += `Chapters:\n${chapters}\n\n`;
    }
...
/* ------------------------------------------------------------------ */
export function buildReviewHtml(title, results, timeline, hasMaster, voiceId, mock) {
...
export async function writeAllOutputs(opts) {
    const { outputDir, title, voiceId, voiceName, template, language, mock, results, segments, hasMaster, muxResult, } = opts;
    // Manifest
