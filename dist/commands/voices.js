...
/*  Command handler                                                    */
/* ------------------------------------------------------------------ */
export async function voicesCommand(opts) {
    ...
    console.log(chalk.bold('╚══════════════════════════════════════════════════╝'));
    if (isMock)
        console.log(chalk.yellow('   ⚡ Mock mode\n'));
    try {
        const { voices, total } = await client.listVoices({ limit, query });
        if (voices.length === 0) {
            console.log(chalk.yellow('   No voices found.'));
            if (query)
                console.log(chalk.gray(`   Try a different search: --query "narrator"`));
            console.log('');
            return;
        }
        // Column widths
        ...
        // Header
        console.log(chalk.gray(`   ${'ID'.padEnd(idW)}  ${'NAME'.padEnd(nameW)}  LANG  DESCRIPTION`));
        ...
        if (query)
            console.log(chalk.gray(`   Filter: "${query}"`));
