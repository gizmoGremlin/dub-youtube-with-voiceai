...
import { checkFfmpeg, stitchSegments, encodeToMp3, normalizeLoudness, muxAudioVideo, generateMuxScripts, } from '../ffmpeg.js';
...
export async function buildCommand(opts) {
    ...
    if (!isMock && !apiKey) {
        console.error(chalk.red('✗ VOICE_AI_API_KEY not set.\n') +
    ...
    console.log(chalk.bold('╚══════════════════════════════════════════════════╝'));
    if (isMock)
        console.log(chalk.yellow.bold('   ⚡ Mock mode — no API calls will be made\n'));
    ...
    if (opts.video)
        console.log(chalk.gray(`   Video:    ${opts.video}`));
    // ── Initialize client ─────────────────────────────────────────────
    ...
    if (ffAvail.ffmpeg) {
        ...
        if (stitched) {
            ...
            if (mp3Ok)
                console.log(chalk.green(`   ✅ master.mp3`));
            ...
            if (normOk)
                console.log(chalk.green(`   ✅ master_normalized.wav (loudnorm -16 LUFS)`));
        }
        else {
    ...
    if (muxResult?.success)
        console.log(chalk.gray(`   Video:     ${muxResult.outputPath}`));
