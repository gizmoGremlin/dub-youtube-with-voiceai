...
import { checkFfmpeg, muxAudioVideo, generateMuxScripts, } from '../ffmpeg.js';
import { fileExists, writeOutputFile, ensureDir } from '../utils.js';
/* ------------------------------------------------------------------ */
/*  Command handler                                                    */
/* ------------------------------------------------------------------ */
export async function replaceAudioCommand(opts) {
    // ── Validate inputs ────────────────────────────────────────────────
    if (!opts.video) {
    ...
    if (!['shortest', 'pad', 'trim'].includes(syncPolicy)) {
        console.error(chalk.red(`✗ Invalid sync policy: "${syncPolicy}". Use: shortest, pad, or trim.`));
        process.exit(1);
    }
...
//# sourceMappingURL=replace-audio.js.map